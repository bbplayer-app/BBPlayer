...
- 修复选择「启动时打开页面」不生效的问题
- 修复音乐库标签页始终停留在第一页的问题
- 修复正在播放条在部分场景下模态框显示异常的问题
- 修复上报播放记录失败时弹出提示的问题
- 修复获取视频收藏状态时数据为 `null` 导致的报错
- 修复本地歌单全选/反选失效的问题
- 修复在部分分页播放列表页面错误显示全选/反选功能的问题
- 修复 WavySlider 在切歌时会出现 use-after-free 的问题
- 避免将音源、网络或设备侧的播放器报错上报到 sentry
- 修复删除歌单时因竞态导致的 404 问题
- 有条件地渲染歌单页选择框，优化性能
- 在搜索页面过滤掉“课堂”类型的视频
- 修复在播放列表为空时切换随机模式时报错
...
- 优化进入播放列表页面时的性能
- 优化播放器页面性能
- 将 NowPlayingBar 挂载到根布局，并使用 `react-native-ease` 执行布局动画
- 播放速度调节由按钮改为滑块
- 升级 TrueSheet 到 v4
- 音乐库改用更轻量的 Skeleton 实现
- 在根布局引入 react-freeze，冻结非活动标签页
- 升级 `react-native-pager-view` 到 9.0.4
- 功能菜单完全迁移到 Expo UI
- 优化远程歌单头部的操作按钮
- 移除分页歌单的全选功能
- 同步上游 react-native-bottom-tabs 依赖
...
- 播放设置中新增「允许与其他软件同时播放」开关，关闭后音频焦点独占不被其他应用打断

### Fixed

- 修复了在按键导航栏模式下 NowPlayingBar 显示错误的问题
- 修复因竞态导致的闪退问题
- 修复 SplashScreen 视频播放卡死的问题
- 修复搜索装扮时结果为空导致的报错问题
- 避免将网络问题导致的播放器报错上报到 sentry
- 修复 PlayerLyrics 在歌词解析失败时弹出 toast 的问题，并使用 useMemo 缓存解析结果避免切换页面时重复解析
- 修复 bottom tabs 在软键盘升起时被错误抬升的问题
- 修复因图片过大导致软件崩溃的问题
...
[2.6.0]: https://github.com/bbplayer-app/BBPlayer/compare/v2.5.2...v2.6.0
