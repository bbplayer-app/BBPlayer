...
import type { BilibiliTrack, Playlist } from '@/types/core/media'
...
export class SyncFacade {
	 ...
	 * @returns
	 */
	public addTrackFromBilibiliApi(bvid: string, cid?: number) {
		logger.info('开始添加 Track（Bilibili）', { bvid, cid })
		return Sentry.startSpan(
			{
				name: 'SyncFacade.addTrackFromBilibiliApi',
				op: 'function',
			},
			async () =>
				this.bilibiliApi.getVideoDetails(bvid).andThen((data) => {
					const trackPayload = {
						title: data.title,
	...
	public async syncCollection(
		...
		const syncId = `collection::${collectionId}`
		if (this.syncingIds.has(syncId)) {
		...
		this.syncingIds.add(syncId)

		return Sentry.startSpan(
			{
				name: 'SyncFacade.syncCollection',
				op: 'function',
				attributes: { syncType: 'collection' },
			},
			async (span) => {
				try {
					...
					return this.bilibiliApi
						...
						.andThen((contents) => {
							...
							return ResultAsync.fromPromise(
								this.db.transaction(async (tx) => {
									...
									const artistRes =
										await artistSvc.findOrCreateManyRemoteArtists(
						...
						.andTee(() => span.setStatus({ code: SPAN_STATUS_OK }))
						.orTee(() => span.setStatus({ code: SPAN_STATUS_ERROR }))
				} finally {
	...
	public async syncMultiPageVideo(
		...
		return Sentry.startSpan(
			{
				name: 'SyncFacade.syncMultiPageVideo',
				op: 'function',
				attributes: { syncType: 'multi_page' },
			},
			async (span) => {
				try {
	...
	public async syncFavorite(
		...
		const syncId = `favorite::${favoriteId}`
		if (this.syncingIds.has(syncId)) {
			logger.info('已有同步任务在进行，跳过', {
				type: 'favorite',
				id: favoriteId,
		...
		return Sentry.startSpan(
			{
				name: 'SyncFacade.syncFavorite',
				op: 'function',
				attributes: { syncType: 'favorite' },
			},
			async (span) => {
				try {
